...
def getNumVars(node, geoPath, geoFormat):
    """临时创建一个 Houdini 节点网络（sopcreate），导入模型文件.

    读取模型中有多少个 primitive（用于判断这个资产有多少“变化体”/Variants）。
    """
...
def createVarNodes(node, i, input):
    """在 Houdini 中为每个 variation 创建节点结构.

    geo_var_i 和 proxy_var_i：子网络，用于放置具体几何体。

    null 节点和 merge、switch 节点：用于组织连接和可视化输出。
    """
...
def createGeoVar(hda, node, assetName, filePath, i, path, type, geoFormat):
    """在每个 variation 的子网络中创建具体的几何节点.

    读入文件（file）

    调整比例（xform）

    如果是 3D 模型，进行 blast、pack、convert 操作拆解变体

    可选地使用 matchsize 做归一化

    添加 wrangle 节点写入路径属性（s@path）
...
def buildGeo(
    ...
    创建 addvariant → configureprimitive → setvariant 节点链。

    根据资产类型：

    如果是 3D：直接从文件夹读取多变体几何。

    如果是 3DPlant：遍历 Var1, Var2, Var3 子文件夹。

    调用 createVarNodes() + createGeoVar() 为每个变体构建几何。
    ...
    megascansSubnet = node.node("PB_megascans_variants")
    ...
    if assetType == "3D":
        ...
        geoPath, proxyPath = getGeoAndProxyPath(
            assetObjFolderList,
            geoFormat,
            lod,
            lodProxy,
        ...
        for i in range(numVars):
            # Create geo var
            geoVarCreate, proxyVarCreate = createVarNodes(
                megascansSubnet,
                i,
                addvariant,
            )
...
# 自动生成 MaterialX 网络、读取贴图、连接材质


# Create MaterialX subnet
def createMaterialXSubnet(parent):
    """在 materiallibrary1 下创建一个 MaterialX 材质子网（mtlxmaterial__VAR_1）.
    ...
    hou_parm_template2 = hou.StringParmTemplate(
        ...
        default_value=(
            [
                "karma USD ^mtlxramp* ^hmtlxramp* ^hmtlxcubicramp* MaterialX parameter constant collect null genericshader subnet subnetconnector suboutput subinput",
            ]
        ),
        naming_scheme=hou.parmNamingScheme.Base1,
        string_type=hou.stringParmType.Regular,
    ...
    mtlxstandard_surface = mtlx_subnet.createNode(
        "mtlxstandard_surface",
        "mtlxstandard_surface",
    )
    output.setInput(0, mtlxstandard_surface)

    displacementOutput = mtlx_subnet.createNode(
        "subnetconnector",
        "displacement_output",
    )
...
def getTexturesDict(texturesNames, texturesDir, res, textFormat, lod):
    """扫描贴图文件夹，匹配出每种贴图类型的文件路径.

    Albedo, Metalness, Roughness, Normal, AO, Displacement, Opacity, Transmission, Translucency

    根据 LOD、分辨率和文件格式（jpg/png/exr）筛选。
    ...
    for tn in texturesNames:
        ...
        for tp in textures:
            ...
            if tn in tp and "Normal" in tp and "LOD" in tp:
                ...
                lod != "High"
                and tn in tp
                and "Displacement" in tp
                and textFormat != format
                and format == "exr"
            ):
                if res in tp:
                    texture = tp
...
def getMinMaxPixelValue(path):
    """创建一个临时的 Houdini 几何网络，使用 attribfrommap 加载一张贴图.

    计算像素值的最小值和最大值，用于调整位移贴图（Displacement）的范围。
    """
...
def applyTextures(
    ...
    """根据贴图字典创建 mtlximage 节点并连接.

    Albedo → mtlxmultiply（乘 AO）

    Metalness / Roughness → standard surface 对应输入

    Normal → normalmap

    Displacement → displacement + remap range（缩放）

    Opacity / Transmission / Translucency → 各自对应输入口
    """
...
def buildMaterial(node, assetDir, lod, res, textFormat, assetType, hasProxy):
    ...
    调用 createMaterialXSubnet()；

    收集贴图信息；

    设置 assignmaterial 节点，将材质赋给对应 prim 路径；

    调用 applyTextures() 自动接贴图。
    """
    ...
    mtlx_subnet, mtlxstandard_surface, mtlxdisplacement = createMaterialXSubnet(
        materiallibrary,
    )
    ...
    assignmaterial.parm("primpattern1").set(
        '/`chs("../../CONTROL/name")`' + '`chs("../../geoPrimPath")`',
    )
    assignmaterial.parm("matspecpath1").set(
        '/`chs("../../CONTROL/name")`/materials/mtlxmaterial__VAR_1',
    )
    if hasProxy:
        assignmaterial.parm("nummaterials").set(2)
        assignmaterial.parm("primpattern2").set(
            '/`chs("../../CONTROL/name")`' + '`chs("../../proxyPrimPath")`',
        )
        assignmaterial.parm("matspecpath2").set(
            '/`chs("../../CONTROL/name")`/materials/mtlxmaterial__VAR_1',
        )
...
def getAssetType(assetDir):
    """判断资产类型（通过是否含有 Textures 文件夹区分 3DPlant 与普通 3D）."""
...
def createBuildDialog():
    ...
    if os.path.isdir(assetDir) == False:
        hou.ui.displayMessage(
            "Error: Asset folder does not exists.",
    ...
    if len(assetName) <= 1:
        hou.ui.displayMessage(
            "Error: Asset Name should contain 2 or more letters.",
            severity=hou.severityType.Error,
        )
        return
    ...
    LOD, geoFormat, textRes, textFormat, assetTexturesFolder, assetObjFolder = getAssetInfo(
        assetDir,
        assetType,
    )
    ...
    if len(LOD) == 0 or len(geoFormat) == 0:
        hou.ui.displayMessage(
            "Error: Can't find any abc, fbx or obj geometry.",
            severity=hou.severityType.Error,
        )
        return
...
# Main button function, check errors and create dialog
def clear():
    """清空构建的几何与材质节点，恢复到初始状态."""
    # Get node parameters
    node = hou.pwd()
    geo_subnet = node.node("PB_megascans_variants")
